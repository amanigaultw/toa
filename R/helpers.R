geneScreen <- function(expression_data, exp_symbol_col = 1, screenSD = 0, verbose = FALSE){
  ...
  keep <- apply(expression_data[,-exp_symbol_col], 1, function(x) ifelse(sd(x) <= screenSD, 0, 1))
...
get_final_diag_scores <- function(raw_diag_scores, pop_gene_symbols){
  ...
  raw_diag_scores_agg <- aggregate(raw_diag_scores[,-1], list(raw_diag_scores[,1]), mean)
...
get_boot_DEG <- function(analysis_data, regressor_matrix, foldThreshDEG, n_boot, verbose){
  ...
  boot_DEG = foreach::foreach(i=1:n_boot, .inorder=FALSE, .options.snow = opts, .export = c("get_df_DEG", "get_x_cov")) %dopar% {
    ...
    DEG <- get_df_DEG(x = get_x_cov(analysis_data, regressor_matrix)$x[sampled_rows],
                      cov = get_x_cov(analysis_data, regressor_matrix)$cov[sampled_rows,],
...
get_boot_stats <- function(boot_means_matrix, toa_means){
  ...
  boot_se <- list(boot_se_toa_upreg_linear = sd(boot_means_matrix[,1]),
                  boot_se_toa_downreg_linear = sd(boot_means_matrix[,2]),
                  boot_se_toa_reg_linear = sd(boot_means_matrix[,3]),
                  boot_se_toa_upreg_log = sd(boot_means_matrix[,4]),
                  boot_se_toa_downreg_log = sd(boot_means_matrix[,5]),
                  boot_se_toa_reg_log = sd(boot_means_matrix[,6]))
